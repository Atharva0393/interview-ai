      ...
      <div className="space-y-4">
        {recommendations.map((rec) => {
          ...
          return (
            <div
              key={rec.priority}
              className="p-5 rounded-2xl bg-stone-50 border border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-violet-300 transition-colors"
            >
              <div className="flex items-start gap-4">
                <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold border shrink-0 bg-violet-50 text-violet-800 border-violet-200">
                  {rec.priority}
                </span>

                <div className="space-y-1">
                  <h4 className="text-sm font-bold text-slate-900 group-hover:text-violet-700 transition-colors flex items-center gap-2">
              ...
              {rec.actionHref && (
                <Link
                  href={rec.actionHref}
                  className="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold transition-all shadow-2xs flex items-center gap-1.5 shrink-0 self-start sm:self-auto cursor-pointer"
                >
