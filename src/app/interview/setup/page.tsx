  ...
  return (
    <AppShell>
      <div className="space-y-8 max-w-7xl mx-auto animate-in fade-in duration-300 relative">
        {/* Page Header & Stepper */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-[#e5e3de] pb-6">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-50 border border-violet-100 text-xs font-semibold text-violet-800 mb-1">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 bg-white border border-[#e5e3de] p-2 rounded-2xl overflow-x-auto text-xs shrink-0 shadow-2xs">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-violet-600 text-white font-bold shadow-2xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  return (
                    <button
                      ...
                      className={`p-3.5 rounded-xl border text-left text-xs font-bold transition-all cursor-pointer flex items-center justify-between ${
                        isSelected
                          ? "bg-violet-50/80 border-violet-600 text-violet-900 shadow-2xs ring-1 ring-violet-500/20"
                          : "bg-white border-stone-200 text-slate-600 hover:border-stone-300 hover:bg-stone-50"
                      }`}
                    >
                      <span>{role}</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed font-normal">{opt.desc}</p>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {difficultyOptions.map((diff) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-extrabold text-slate-900">{diff.title}</span>
                        {isSelected && <Check className="w-4 h-4 text-violet-600" />}
                      </div>
                      <p className="text-[11px] text-slate-500 leading-relaxed font-normal">{diff.desc}</p>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {durationOptions.map((dur) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-900">{dur.title}</span>
                        {isSelected && <Check className="w-4 h-4 text-violet-600" />}
                      </div>
                      <p className="text-[11px] font-bold text-violet-700">{dur.count}</p>
                    </button>
                  );
                })}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer ${
                        isSelected
                          ? "bg-violet-600 text-white shadow-2xs border border-violet-600"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-xl bg-stone-50 border border-stone-200 flex items-start justify-between gap-4">
                  ...
                  <button
                    ...
                    className={`w-11 h-6 rounded-full transition-colors relative shrink-0 p-0.5 cursor-pointer ${
                      cameraAnalysis ? "bg-violet-600" : "bg-stone-300"
                    }`}
                  >
                    <div
                ...
                <div className="p-4 rounded-xl bg-stone-50 border border-stone-200 flex items-start justify-between gap-4">
                  ...
                  <button
                    ...
                    className={`w-11 h-6 rounded-full transition-colors relative shrink-0 p-0.5 cursor-pointer ${
                      voiceAnalysis ? "bg-violet-600" : "bg-stone-300"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 rounded-full bg-white shadow-2xs transition-transform ${
              ...
              <div className="space-y-3 text-xs">
                <div className="p-3.5 rounded-xl bg-stone-50/70 border border-stone-200 space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded font-bold bg-violet-100 text-violet-800 border border-violet-200 text-[10px]">
                      Technical
                    </span>
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-20 p-6 rounded-2xl bg-white border border-[#e5e3de] shadow-2xs space-y-6">
              <div>
                <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              <div className="space-y-3.5 text-xs">
                ...
                <div className="flex items-center justify-between pb-2 border-b border-stone-100">
                  ...
                  <span className="text-violet-700 font-bold">{selectedType}</span>
                </div>
                ...
                <div className="pb-2 border-b border-stone-100 space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {focusAreas.map((f) => (
                      <span key={f} className="px-2 py-0.5 rounded bg-violet-50 text-violet-700 border border-violet-100 text-[10px] font-bold">
                        {f}
                      </span>
                    ))}
                  </div>
        ...
        {isPreparing && (
          <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="max-w-md w-full p-8 rounded-3xl bg-white border border-[#e5e3de] text-center space-y-6 shadow-xl relative">
              <div className="w-16 h-16 rounded-2xl bg-violet-50 border border-violet-200 p-0.5 mx-auto flex items-center justify-center text-violet-600 shadow-2xs">
              ...
              <div className="space-y-2">
                ...
                <h3 className="text-xl font-extrabold text-slate-900">Preparing Your Session</h3>
                <p className="text-xs text-violet-700 font-mono font-bold">
                  {prepSteps[prepStep]}
                </p>
              </div>
